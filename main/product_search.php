  ...
  <body>
    <?if($_SESSION['userid']){?>
      ...
      <div class="content">
        ...
        $query = mysqli_query($con,"select * from list where subject LIKE '%$search%'");
        ...
        $result = mysqli_query($con,"select * from list where subject LIKE '%$search%' order by number desc limit $first_page,$list");
              ...
              $i = 0;?>
                <tr>
                  ...
                  while ($row = mysqli_fetch_array($result)) {
                    if ($i %2 == 0) { ?><tr> <?}?>
                      <td><a href="pro_detail.php?number=<?=$row['number']?>&list=<?=$row['list']?>"><img src="<?echo $row['image'].'.'.$row['ext'];?>" alt="image"></a></td>
                      <td width=100><img src="subject.jpg"  alt="subject"> <?echo $row['subject'];?></td>
                      <td width=50><img src="price.jpg"  alt="price"> <?echo $row['price'];?></td>
        ...
        <center><?
        if($pageNum>=2){?>
          <a href="product_search.php?page=<?=$b_start_page?>&list=<?=$list?>&word=<?=$search?>"><img src="first2.jpg" alt="first"></a>
          <a href="product_search.php?page=<?=$pageNum-1?>&list=<?=$list?>&word=<?=$search?>"><img src="prev2.jpg" alt="prev"></a>
          <?}
        for($j = $b_start_page; $j <=$b_end_page; $j++){
          if($pageNum == $j){?>
            <?=$j?>
          <?}else{?>
              <font size=4><a href="product_search.php?page=<?=$j?>&list=<?=$list?>&word=<?=$search?>"><?=$j?></a></font>
            <?}
        }
        if($pageNum<$b_end_page){?>
          <a href="product_search.php?page=<?=$pageNum+1?>&list=<?=$list?>&word=<?=$search?>"><img src="next2.jpg" alt="next"></a>
          <a href="product_search.php?page=<?=$b_end_page?>&list=<?=$list?>&word=<?=$search?>"><img src="last2.jpg" alt="last"></a>
          <?}?></center>
        </font>
      </b></p>
    </div>
