    ...
    <?if($_SESSION['userid']){?>
      ...
      <div class="container">
        <div class="content">
          ...
          <p class="write"><a href="write.php?list=3"><img src="write.jpg" alt="글쓰기"/></a></p>
        </form><br><br><br><br>
        <?$list=1;
        $con = mysqli_connect("localhost",'root','apmsetup','ssong');
        $query = mysqli_query($con,"select * from list WHERE list=3 ");
        $list = $_GET['list'];
        $page = $_GET['page'];
        $number = mysqli_num_rows($query);//총게시글수
        $first_page = ((($page-1)*$list));

        $result = mysqli_query($con,"select * from list WHERE list=3 order by number desc limit $first_page,$list");
            ...
            $i = 0;?>
              <tr>
                ...
                while ($row = mysqli_fetch_array($result)) {
                  if ($i %2 == 0) { ?><tr> <?}?>
                    <td><a href="pro_detail.php?number=<?=$row['number']?>&list=3"><img  width="100" height="100" src="./images/<?echo $row['image'].'.'.$row['ext'];?>" alt="image"></a></td>
        ...
        <center><?
        if($pageNum>=2){?>
          <a href="pro_list3.php?page=<?=$b_start_page?>&list=<?=$list?>"><img src="first2.jpg" alt="first"></a>
          <a href="pro_list3.php?page=<?=$pageNum-1?>&list=<?=$list?>"><img src="prev2.jpg" alt="prev"></a>
          <?}
        for($j = $b_start_page; $j <=$b_end_page; $j++){
          if($pageNum == $j){?>
            <?=$j?>
          <?}else{?>
              <font size=4><a href="pro_list3.php?page=<?=$j?>&list=<?=$list?>"><?=$j?></a></font>
            <?}
        }
        if($pageNum<$b_end_page){?>
          <a href="main.pro_list3?page=<?=$pageNum+1?>&list=<?=$list?>"><img src="next2.jpg" alt="next"></a>
          <a href="pro_list3.php?page=<?=$b_end_page?>&list=<?=$list?>"><img src="last2.jpg" alt="last"></a>
          <?}?></center>
        </font>
      </b></p>
    </div>
